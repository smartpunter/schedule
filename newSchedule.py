        ...
        for idx, length in enumerate(class_lengths):
            ...
            if fixed is not None:
                ...
                for d_idx, day in enumerate(days):
                    slots = day["slots"]
                    for sl in slots:
                        if sl + length - 1 > max(slots):
                            continue
                        if not all(sl + off in student_limits[s][day["name"]] for s in class_students for off in range(length)):
                            continue
            ...
            if cabinets_list is None:
                cab_choices = [c for c in allowed_cabs if c in cabinets and cabinets[c]["capacity"] >= size]
                vars_c = []
